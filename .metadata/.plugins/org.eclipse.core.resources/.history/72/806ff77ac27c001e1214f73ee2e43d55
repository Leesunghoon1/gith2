...
public class LoginSuccessHandler implements AuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		if(!isOk || ses == null) {
			...
			//이전의 인증 예외를 세션에서 제거합니다. 로그인 시도에 대한 이전의 실패한 인증 예외 기록을 지우는 역할을 합니다.
		}
		...
		//rdstg 객체를 사용하여 사용자를 리디렉션합니다. 이전 요청 정보(saveReq)가 있는 경우, 
		//해당 URL로 리디렉션하고, 그렇지 않으면 getAuthUrl()에서 지정한 기본 URL로 리디렉션합니다.
